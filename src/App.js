...
// --- Funções da API Gemini (Segura) ---
const callGeminiAPI = async (prompt, showNotification) => {
    try {
        const resultText = await window.electronAPI.callGemini(prompt);
        try {
            const potentialError = JSON.parse(resultText);
            if (potentialError && potentialError.error) {
                const errorMessage = potentialError.error.message || "Erro desconhecido da API Gemini";
                showNotification(`Erro ao gerar texto com IA: ${errorMessage}`, 'error');
                return null;
            }
        } catch (e) {
            // Se não for um JSON de erro, é uma resposta de texto válida.
        }
        return resultText;
    } catch (error) {
        console.error('Erro ao chamar a API Gemini via IPC:', error);
        showNotification(`Erro de comunicação com a IA: ${error.message}`, 'error');
        return null;
    }
};
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title={title} size="sm">
            ...
            <div className="flex justify-end space-x-3">
                <button onClick={onClose} className="py-2 px-4 bg-slate-600 hover:bg-slate-500 rounded-md text-white transition-colors">{cancelText}</button>
...
const MasterDashboard = ({ campaigns, setCampaigns, showNotification }) => {
    ...
    const handleImportCampaign = async () => {
        ...
        if (result.success) {
            ...
            showNotification(`Campanha "${result.campaign.name}" importada com sucesso!`, 'success');
        } else {
            if (result.message !== 'Nenhum ficheiro selecionado.') {
                showNotification(`Erro ao importar: ${result.message}`, 'error');
            }
        }
    ...
    const handleCreateCampaign = async (e) => {
        ...
        try {
            await window.electronAPI.saveCampaign(newCampaign);
            ...
            showNotification("Erro ao salvar a nova campanha.", "error");
        }
        setLoadingAction(false);
    };
    ...
    const executeDeleteCampaign = async () => {
        ...
        setLoadingAction(true);
        try {
            await window.electronAPI.deleteCampaign(campaignToDelete.id);
            ...
            showNotification("Erro ao excluir a campanha.", "error");
        }
    ...
    const updateCampaignInList = (updatedCampaign) => {
        ...
        window.electronAPI.saveCampaign(updatedCampaign);
    };
    ...
    if (selectedCampaign) {
        return <ManageLocalCampaign 
                    ...
                    updateCampaignInList={updateCampaignInList}
                    showNotification={showNotification} 
                    goBack={() => setSelectedCampaign(null)} 
                    onDeleteCampaignRequest={() => requestDeleteCampaign(selectedCampaign)} 
                />;
    }
    ...
    return (
        <div className="p-4 md:p-6">
            <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
                ...
                <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
                    <button onClick={handleImportCampaign} 
                        className="bg-sky-600 hover:bg-sky-700 text-white font-bold py-2 px-4 rounded-lg transition duration-150 shadow-md hover:shadow-lg shadow-sky-500/20">
                        Importar Campanha
                    </button>
                    <button onClick={() => { setShowCreateModal(true); setAiCampaignIdeas([]); setCampaignTheme(''); setNewCampaignName(''); setNewCampaignDesc(''); }} 
                        className="bg-cyan-600 hover:bg-cyan-700 text-white font-bold py-2 px-4 rounded-lg transition duration-150 shadow-md hover:shadow-lg shadow-cyan-500/20">
                        + Nova Campanha
                    </button>
                </div>
            </div>
            ...
            {!loadingAction && campaigns.length === 0 && (
                <div className="text-center py-10 bg-slate-800/50 border border-slate-700 rounded-lg shadow-inner"><svg className="mx-auto h-12 w-12 text-slate-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path vectorEffect="non-scaling-stroke" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 13h6m-3-3v6m-9 1V7a2 2 0 012-2h6l2 2h6a2 2 0 012 2v8a2 2 0 01-2 2H5a2 2 0 01-2-2z" /></svg><h3 className="mt-2 text-xl font-medium text-slate-300">Nenhuma campanha criada</h3><p className="mt-1 text-sm text-slate-500">Crie ou importe sua primeira aventura!</p></div>
            )}
                ...
                {campaigns.map(campaign => (
                    <div key={campaign.id} className="bg-slate-800 p-6 rounded-lg shadow-xl border border-slate-700/50 hover:border-cyan-500/50 hover:shadow-cyan-500/10 transition-all duration-300 transform hover:-translate-y-1">
                        ...
                        <div className="flex flex-col sm:flex-row justify-between items-center gap-2 pt-4 border-t border-slate-700">
                           ...
                           <button onClick={() => requestDeleteCampaign(campaign)} className="w-full sm:w-auto bg-red-600 hover:bg-red-700 text-white py-2 px-3 rounded-md text-sm transition duration-150 shadow-md">Excluir</button>
                        </div>
                    </div>
                ))}
            </div>
            <div className="mt-8 text-center">
              <button onClick={() => window.electronAPI.openCampaignsFolder()} className="text-sm text-cyan-400 hover:underline">
                Abrir pasta de campanhas
              </button>
            </div>
            ...
            <ConfirmModal isOpen={showConfirmDeleteModal} onClose={() => setShowConfirmDeleteModal(false)} title="Excluir Campanha" message={`Tem a certeza de que quer apagar permanentemente a campanha "${campaignToDelete?.name}"?`} onConfirm={executeDeleteCampaign} confirmText="Excluir" />
        </div>
    );
};
...
const ManageLocalCampaign = ({ campaign: initialCampaign, updateCampaignInList, showNotification, goBack, onDeleteCampaignRequest }) => {
    ...
    const handleBackup = async () => {
        const result = await window.electronAPI.backupCampaign(campaign);
        if (result.success) {
            showNotification('Backup da campanha criado com sucesso!', 'success');
        } else {
            if (result.message !== 'Backup cancelado pelo utilizador.') {
    ...
    const openEditImageModal = (e, img) => {
        e.stopPropagation(); setEditingImage(img); setImageName(img.name); setImageDescription(img.description); setImageFile(img.imageUrl); setShowImageModal(true);
    };
    ...
    const openEditNpcModal = (e, npc) => {
        e.stopPropagation(); setEditingNpc(npc); setNpcName(npc.name); setNpcDesc(npc.description); setNpcKeywords(npc.keywords || ''); setNpcImageUrl(npc.imageUrl || ''); setShowNpcModal(true);
    };
    ...
    const handleSaveMonster = (e) => {
        ...
        const monsterData = { name: monsterName, description: monsterDescription, stats: monsterStats, source: 'manual' };
        let updatedBestiary;
        if (editingMonster) {
        ...
        setShowAddMonsterModal(false); setEditingMonster(null); setMonsterName(''); setMonsterDescription(''); setMonsterStats('');
    };

    const openAddMonsterModal = () => {
        setEditingMonster(null); setMonsterName(''); setMonsterDescription(''); setMonsterStats(''); setShowAddMonsterModal(true);
    }
    ...
    const handleSaveCharacterSheet = (e) => {
        ...
        setShowCharacterSheetModal(false); setEditingCharacterSheet(null); setCsPlayerName(''); setCsCharacterName(''); setCsClass(''); setCsLevel(1); setCsAttributes({}); setCsOtherNotes('');
        showNotification(editingCharacterSheet ? "Ficha atualizada!" : "Ficha adicionada!", "success");
    };

    const openEditCharacterSheetModal = (sheet) => {
        setEditingCharacterSheet(sheet); setCsPlayerName(sheet.playerName); setCsCharacterName(sheet.characterName); setCsClass(sheet.class); setCsLevel(sheet.level); setCsAttributes(sheet.attributes || {}); setCsOtherNotes(sheet.otherNotes); setShowCharacterSheetModal(true);
    };

    const requestDeleteItem = (e, type, id, name) => {
        e.stopPropagation(); setItemToDelete({ type, id, name }); setShowConfirmDeleteItemModal(true);
    };

    const executeDeleteItem = () => {
        if (!itemToDelete) return;
        const { type, id } = itemToDelete;
        const fieldMap = { image: 'images', npc: 'npcs', bestiary: 'bestiary', material: 'materials', gmNote: 'gmNotes', characterSheet: 'characterSheetsData', event: 'events' };
        const fieldName = fieldMap[type];
        if (!fieldName) { showNotification("Tipo de item inválido.", "error"); return; }
        const updatedItemsArray = campaign[fieldName]?.filter(item => item.id !== id) || [];
        saveCampaignChanges({ [fieldName]: updatedItemsArray });
        showNotification("Item excluído!", "success"); setItemToDelete(null);
    };
    ...
    const handleViewDetails = async (item, type) => {
        if (type === 'bestiary' && item.source === 'srd' && !item.fullData.translated_pt) {
            ...
            try {
                ...
                setViewingItem({ item: updatedMonster, type: 'bestiary' });
    ...
    const renderTabContent = () => {
        if (!campaign) return <p className="text-slate-400">Campanha não encontrada ou inválida.</p>;
        switch (activeTab) {
            case 'info':
                return ( <div className="bg-slate-800/50 p-4 rounded-lg"> <h3 className="text-xl font-bold text-cyan-300 font-orbitron">Detalhes da Campanha</h3> <p className="text-slate-300 mt-1 whitespace-pre-line">{campaign.description || "Sem descrição."}</p> <p className="text-xs text-slate-500 mt-2">Criada em: {new Date(campaign.createdAt).toLocaleDateString()}</p> {campaign.updatedAt && <p className="text-xs text-slate-500">Última atualização: {new Date(campaign.updatedAt).toLocaleString()}</p>} </div> );
            case 'images':
                return ( <div> <div className="flex flex-wrap gap-2 mb-4"> <button onClick={() => { setEditingImage(null); setImageName(''); setImageDescription(''); setImageFile(null); setShowImageModal(true);}} className="bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-4 rounded shadow-md">Adicionar Imagem</button> </div> <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4"> {(campaign.images || []).map(img => (<div key={img.id} onClick={() => handleViewDetails(img, 'image')} className="p-3 rounded-lg shadow cursor-pointer transition-all duration-200 hover:ring-2 hover:ring-cyan-500 bg-slate-800 border border-slate-700"> <img src={img.imageUrl} alt={img.name} className="w-full h-40 object-cover rounded mb-2 border border-slate-600"/> <h4 className="text-cyan-300 font-semibold truncate mb-1">{img.name}</h4> <p className="text-slate-400 text-xs line-clamp-3 h-12 overflow-hidden whitespace-pre-line">{img.description}</p> <div className="mt-2 flex justify-end gap-2"> <button onClick={(e) => openEditImageModal(e, img)} className="text-blue-400 hover:underline text-xs">Editar</button> <button onClick={(e) => requestDeleteItem(e, 'image', img.id, img.name)} className="text-red-400 hover:underline text-xs">Excluir</button> </div> </div>))} </div> {(campaign.images || []).length === 0 && <p className="text-slate-400 italic">Nenhuma imagem.</p>} </div> );
            case 'bestiary':
                return ( <div> <div className="flex flex-wrap gap-2 mb-4"> <button onClick={() => openAddMonsterModal()} className="bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-4 rounded shadow-md">Adicionar Monstro Manualmente</button> <button onClick={() => { setGeneratedMonster(null); setMonsterGenKeywords(''); setShowMonsterGeneratorModal(true); }} className="bg-teal-600 hover:bg-teal-700 text-white font-bold py-2 px-4 rounded shadow-md">✨ Gerar Monstro com IA</button> <button onClick={() => setShowImportMonsterModal(true)} className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded shadow-md">Importar Monstro do SRD</button> </div> <div className="grid grid-cols-1 md:grid-cols-2 gap-4"> {(campaign.bestiary || []).map(monster => ( <div key={monster.id} onClick={() => handleViewDetails(monster, 'bestiary')} className={`p-4 rounded-lg shadow cursor-pointer transition-all duration-200 hover:ring-2 ${monster.source === 'ai' ? 'bg-red-900/40 border border-red-700 hover:ring-red-500' : monster.source === 'srd' ? 'bg-indigo-900/40 border border-indigo-700 hover:ring-indigo-500' : 'bg-slate-800/80 border border-slate-700 hover:ring-cyan-500'}`}> <h4 className={`font-bold text-lg ${monster.source === 'ai' ? 'text-red-300' : monster.source === 'srd' ? 'text-indigo-300' : 'text-cyan-300'}`}>{monster.name} <span className="text-xs text-slate-400">({monster.source})</span></h4> <p className="text-sm text-slate-300 mt-2 line-clamp-3 h-16 overflow-hidden whitespace-pre-line">{monster.description || (monster.fullData && 'Clique para ver detalhes...')}</p> <div className="mt-3 flex justify-end"> <button onClick={(e) => requestDeleteItem(e, 'bestiary', monster.id, monster.name)} className="text-red-400 hover:underline text-xs">Excluir</button> </div> </div> ))} </div> {(campaign.bestiary || []).length === 0 && <p className="text-slate-400 italic">Nenhum monstro no bestiário.</p>} </div> );
             case 'npcs':
                return ( <div> <button onClick={() => { setEditingNpc(null); setNpcName(''); setNpcDesc(''); setNpcKeywords(''); setNpcImageUrl(''); setShowNpcModal(true); }} className="bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-4 rounded shadow-md">Adicionar NPC</button> <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mt-4"> {(campaign.npcs || []).map(n => (<div key={n.id} onClick={() => handleViewDetails(n, 'npc')} className="bg-slate-800 border border-slate-700 p-3 rounded-lg shadow cursor-pointer transition-all duration-200 hover:ring-2 hover:ring-cyan-500"> <h4 className="text-cyan-300 font-semibold truncate mb-1">{n.name}</h4> {n.imageUrl && <img src={n.imageUrl} alt={n.name} className="w-full h-32 object-cover rounded my-1 border border-slate-600" onError={(e) => e.target.style.display='none'}/>} <p className="text-slate-400 text-xs line-clamp-3 h-12 overflow-hidden my-1 whitespace-pre-line">{n.description}</p> <p className="text-slate-500 text-xs truncate">Palavras-chave: {n.keywords || "N/A"}</p> <div className="mt-2 flex justify-end gap-2"> <button onClick={(e) => openEditNpcModal(e, n)} className="text-blue-400 hover:underline text-xs">Editar</button> <button onClick={(e) => requestDeleteItem(e, 'npc', n.id, n.name)} className="text-red-400 hover:underline text-xs">Excluir</button> </div> </div>))} </div> {(campaign.npcs || []).length === 0 && <p className="text-slate-400 italic mt-4">Nenhum NPC.</p>} </div> );
            case 'materials':
                return ( <div> <button onClick={() => { setEditingMaterial(null); setMaterialName(''); setMaterialNotes(''); setShowMaterialModal(true); }} className="bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-4 rounded shadow-md">Adicionar Material</button> <ul className="space-y-3 mt-4"> {(campaign.materials || []).map(mat => (<li key={mat.id} className="bg-slate-800 border border-slate-700 p-3 rounded-lg shadow flex justify-between items-start"><div><h4 className="text-cyan-300 font-medium">{mat.name}</h4><p className="text-slate-400 text-xs whitespace-pre-line">{mat.notes}</p></div><div className="flex gap-2 shrink-0 ml-2"><button onClick={(e) => { e.stopPropagation(); openEditMaterialModal(mat); }} className="text-blue-400 hover:underline text-xs">Editar</button><button onClick={(e) => requestDeleteItem(e, 'material', mat.id, mat.name)} className="text-red-400 hover:underline text-xs">Excluir</button></div></li>))} </ul> {(campaign.materials || []).length === 0 && <p className="text-slate-400 italic mt-4">Nenhum material.</p>} </div> );
            case 'gmNotes':
                 return ( <div> <button onClick={() => { setEditingGmNote(null); setGmNoteContent(''); setShowGmNoteModal(true);}} className="bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-4 rounded shadow-md">+ Nova Nota</button> <div className="space-y-4 mt-4"> {(campaign.gmNotes || []).map(note => (<div key={note.id} className="bg-slate-800 border border-slate-700 p-4 rounded-lg shadow"><p className="text-slate-300 whitespace-pre-wrap">{note.content}</p><div className="flex justify-between items-center mt-2 pt-2 border-t border-slate-600"><small className="text-slate-500 text-xs">Criada: {new Date(note.createdAt).toLocaleString()}{note.updatedAt && ` (Atualizada: ${new Date(note.updatedAt).toLocaleString()})`}</small><div className="flex gap-2"><button onClick={(e) => { e.stopPropagation(); openEditGmNoteModal(note); }} className="text-blue-400 hover:underline text-xs">Editar</button><button onClick={(e) => requestDeleteItem(e, 'gmNote', note.id, 'esta nota')} className="text-red-400 hover:underline text-xs">Excluir</button></div></div></div>))} </div> {(campaign.gmNotes || []).length === 0 && <p className="text-slate-400 italic mt-4">Nenhuma nota.</p>} </div> );
            case 'characterSheetsData': 
                return ( <div> <button onClick={() => { setEditingCharacterSheet(null); setCsPlayerName(''); setCsCharacterName(''); setCsClass(''); setCsLevel(1); setCsAttributes({}); setCsOtherNotes(''); setShowCharacterSheetModal(true);}} className="bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-4 rounded mb-4 shadow-md">Adicionar Ficha de Personagem</button> <div className="grid grid-cols-1 md:grid-cols-2 gap-4"> {(campaign.characterSheetsData || []).map(sheet => ( <div key={sheet.id} className="bg-slate-800 border border-slate-700 p-4 rounded-lg shadow"> <div className="flex justify-between items-start"> <div> <h4 className="text-cyan-300 font-semibold">{sheet.characterName}</h4> <p className="text-xs text-slate-400">{sheet.playerName || 'Jogador Desconhecido'}</p> </div> <div className="text-right"> <p className="text-sm font-medium">{sheet.class || 'Classe'}{sheet.level ? ` - Nv. ${sheet.level}` : ''}</p> <p className="text-xs text-slate-400">{sheet.attributes?.race || 'Raça'}</p> </div> </div> <div className="mt-3 pt-3 border-t border-slate-600/50 grid grid-cols-2 gap-x-4 gap-y-1 text-sm"> <p><strong>PV:</strong> {sheet.attributes?.hp_current || '?'}/{sheet.attributes?.hp_max || '?'}</p> <p><strong>CA:</strong> {sheet.attributes?.ac || '?'}</p> <p><strong>Desloc.:</strong> {sheet.attributes?.speed || '?'}</p> <p><strong>Iniciativa:</strong> {sheet.attributes?.initiative || '?'}</p> </div> <div className="mt-3 flex justify-end gap-2"> <button onClick={(e) => { e.stopPropagation(); openEditCharacterSheetModal(sheet); }} className="text-blue-400 hover:underline text-xs">Editar</button> <button onClick={(e) => requestDeleteItem(e, 'characterSheet', sheet.id, sheet.characterName)} className="text-red-400 hover:underline text-xs">Excluir</button> </div> </div> ))} </div> {(campaign.characterSheetsData || []).length === 0 && <p className="text-slate-400 italic mt-4">Nenhuma ficha de personagem anotada.</p>} </div> );
            default: return null;
        }
    };
        ...
        <div className="p-4 md:p-6">
            <div className="flex justify-between items-center mb-6">
                ...
                <div className="flex gap-2">
                    <button onClick={handleBackup} className="bg-yellow-600 hover:bg-yellow-700 text-white font-bold py-2 px-3 rounded-lg shadow-md whitespace-nowrap">
                        Fazer Backup
                    </button>
                    <button onClick={onDeleteCampaignRequest} className="bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-3 rounded-lg shadow-md whitespace-nowrap">
                        Excluir Campanha
                    </button>
                </div>
            </div>
            
            <h2 className="text-3xl font-bold font-orbitron text-cyan-300 break-words mb-6">{campaign?.name || "Campanha sem nome"}</h2>
...
const App = () => {
    ...
    useEffect(() => {
        setIsLoading(true);
        window.electronAPI.getCampaigns().then(loadedCampaigns => {
                ...
                <main className="container mx-auto p-2 sm:p-4">
                 ...
                 <footer className="text-center py-8 text-slate-500 text-sm border-t border-slate-800/50 mt-12">
                    <p>&copy; {new Date().getFullYear()} StarForge RPG Manager. Criado por RaryPoters.</p>
                     <p className="text-xs mt-1">Dica: As suas campanhas estão salvas na pasta Documentos/StarForge/Campaigns.</p>
                </footer>
            </div>
        </>
    );
...
const DndResourcesView = ({ showNotification }) => {
    ...
    const schoolTranslations = { 'Abjuration': 'Abjuração', 'Conjuration': 'Conjuração', 'Divination': 'Adivinhação', 'Enchantment': 'Encantamento', 'Evocation': 'Evocação', 'Illusion': 'Ilusão', 'Necromancy': 'Necromancia', 'Transmutation': 'Transmutação' };
    const getTranslatedSchool = (schoolName) => schoolTranslations[schoolName] || schoolName;
    
    const tavernItems = {
        foods: [ { name: "Ensopado do Viajante", price: "3 pc", desc: "Um guisado simples e quente de raízes e legumes, com pedaços de carne de origem duvidosa. Reconfortante." }, { name: "Pão com Queijo", price: "2 pc", desc: "Uma fatia de pão escuro e um pedaço de queijo duro. Sustenta, mas não inspira." }, { name: "Leitão Assado (fatia)", price: "8 pp", desc: "Uma porção generosa de leitão assado com pele crocante e ervas. Um luxo para o viajante." }, { name: "Sopa de Nabo", price: "1 pc", desc: "Uma sopa aguada com mais nabo do que qualquer outra coisa. Pelo menos é quente." } ],
        drinks: [ { name: "Cerveja Comum", price: "4 pc", desc: "Uma caneca de cerveja local, aguada mas refrescante." }, { name: "Vinho (taça)", price: "2 pp", desc: "Um vinho tinto robusto da região. Deixa uma mancha roxa nos lábios." }, { name: "Hidromel", price: "5 pc", desc: "Doce e forte, uma bebida favorita entre anões e bárbaros." }, { name: "Água", price: "Grátis", desc: "Pode vir do poço ou da chuva. Beba por sua conta e risco." } ]
    };
    ...
    useEffect(() => {
        const fetchListData = async () => {
            ...
            try {
                ...
                if (translatedNamesString && !translatedNamesString.includes("error")) {
                    const translatedNames = translatedNamesString.split(', ');
                    if (translatedNames.length === data.results.length) {
                        translatedData = data.results.map((item, index) => ({...item, name_pt: translatedNames[index]}));
                    }
                }
    ...
    useEffect(() => {
        const dataToFilter = activeTab === 'tavern' ? [...tavernItems.foods, ...tavernItems.drinks] : allData[activeTab];
        if (!searchTerm) { setFilteredData(dataToFilter); } 
        else { setFilteredData((dataToFilter || []).filter(item => (item.name && item.name.toLowerCase().includes(searchTerm.toLowerCase())) || (item.name_pt && item.name_pt.toLowerCase().includes(searchTerm.toLowerCase()))));}
    ...
    const handleSelectItem = async (itemUrl, isStatic = false, staticItem = null) => {
        if (isStatic) { setSelectedItemDetails(staticItem); return; }
        setIsLoadingDetails(true);
        ...
        try {
            ...
            let finalData = data;
            if (translatedJsonString && !translatedJsonString.includes("error")) {
                try {
                    const jsonMatch = translatedJsonString.match(/(\{[\s\S]*\})/);
                    if(jsonMatch && jsonMatch[0]) { finalData = { ...data, ...JSON.parse(jsonMatch[0]) }; } 
                    else { throw new Error("Resposta da IA não continha um JSON válido."); }
                } catch(e) { showNotification("Falha ao processar tradução da IA. Exibindo em inglês.", "warning"); }
            }
    ...
    const renderDetails = (item) => {
        if(!item) return null;
        const attributeTranslations = { strength: 'FOR', dexterity: 'DES', constitution: 'CON', intelligence: 'INT', wisdom: 'SAB', charisma: 'CAR' };
        
        switch(activeTab) {
            case 'spells': return <div className="space-y-2 text-sm text-slate-300"> <p><strong>Nível:</strong> {item.level > 0 ? `${item.level}º Círculo de ` : "Truque de "} {getTranslatedSchool(item.school.name)}</p> <p><strong>Tempo de Conjuração:</strong> {item.casting_time}</p> <p><strong>Alcance:</strong> {formatDistance(item.range)}</p> <p><strong>Componentes:</strong> {item.components.join(', ')} {item.material && `(${item.material.substring(0, 100)}${item.material.length > 100 ? '...' : ''})`}</p> <p><strong>Duração:</strong> {item.duration}</p> <div className="mt-3 pt-3 border-t border-slate-700">{(item.desc || []).map((p, i) => <p key={i} className="mb-2">{p}</p>)}</div> {item.higher_level?.length > 0 && (<div className="mt-3 pt-3 border-t border-slate-700"><p><strong>Em Níveis Superiores:</strong></p>{(item.higher_level).map((p, i) => <p key={i} className="mb-2">{p}</p>)}</div>)} <p className="text-xs text-slate-500 pt-2">Classes: {(item.classes || []).map(c => c.name).join(', ')}</p> </div>;
            case 'monsters': return <div className="space-y-2 text-sm text-slate-300"> <p>{item.size} {item.type}, {item.alignment}</p> <p><strong>Classe de Armadura:</strong> {item.armor_class?.map(ac => `${ac.value} (${ac.type})`).join(', ') || 'N/A'}</p> <p><strong>Pontos de Vida:</strong> {item.hit_points} ({item.hit_dice})</p> <p><strong>Deslocamento:</strong> {formatDistance(item.speed)}</p> <div className="grid grid-cols-6 gap-2 text-center my-2 pt-2 border-t border-slate-700"> {Object.keys(attributeTranslations).map(attr => ( <div key={attr}><p className="font-bold uppercase">{attributeTranslations[attr]}</p><p>{item[attr]} ({(Math.floor((item[attr] - 10) / 2)) >= 0 ? '+' : ''}{Math.floor((item[attr] - 10) / 2)})</p></div> ))} </div> {item.special_abilities?.length > 0 && <div className="pt-2 border-t border-slate-700"><strong>Habilidades Especiais:</strong> {item.special_abilities.map(a => <div key={a.name} className="mt-1"><p className="font-semibold">{a.name}:</p><p>{a.desc}</p></div>)}</div>} {item.actions?.length > 0 && <div className="pt-2 border-t border-slate-700"><strong>Ações:</strong> {item.actions.map(a => <div key={a.name} className="mt-1"><p className="font-semibold">{a.name}:</p><p>{a.desc}</p></div>)}</div>} {item.legendary_actions?.length > 0 && <div className="pt-2 border-t border-slate-700"><strong>Ações Lendárias:</strong> {item.legendary_actions.map(a => <div key={a.name} className="mt-1"><p className="font-semibold">{a.name}:</p><p>{a.desc}</p></div>)}</div>} </div>;
            case 'classes': return <div className="space-y-2 text-sm text-slate-300"> <p><strong>Dado de Vida:</strong> d{item.hit_die}</p> <p><strong>Proficiências:</strong> {item.proficiencies.map(p => p.name).join(', ')}</p> <div className="pt-2 border-t border-slate-700"> <h4 className="font-semibold mb-1">Testes de Resistência:</h4> <p>{item.saving_throws.map(s => s.name).join(', ')}</p> </div> </div>;
            case 'races': return <div className="space-y-2 text-sm text-slate-300"> <p><strong>Aumento de Atributo:</strong> {item.ability_bonuses.map(b => `${b.ability_score.name} +${b.bonus}`).join(', ')}</p> <p><strong>Tendência:</strong> {item.alignment}</p> <p><strong>Idade:</strong> {item.age}</p> <p><strong>Tamanho:</strong> {item.size_description}</p> <p><strong>Deslocamento:</strong> {formatDistance(item.speed)}</p> <div className="pt-2 border-t border-slate-700"> <h4 className="font-semibold mb-1">Traços Raciais:</h4> {item.traits.map(t => <p key={t.index}>- {t.name}</p>)} </div> </div>;
            case 'equipment': return <div className="space-y-2 text-sm text-slate-300"> {item.desc?.map((p, i) => <p key={i}>{p}</p>)} <p><strong>Custo:</strong> {item.cost.quantity} {item.cost.unit}</p> <p><strong>Peso:</strong> {item.weight} lb.</p> {item.damage && <p><strong>Dano:</strong> {item.damage.damage_dice} {item.damage.damage_type.name}</p>} {item.properties?.length > 0 && <p><strong>Propriedades:</strong> {item.properties.map(p => p.name).join(', ')}</p>} </div>;
            case 'tavern': return <div className="space-y-2 text-sm text-slate-300"> <p><strong>Preço:</strong> {item.price}</p> <p className="mt-2">{item.desc}</p> </div>;
            default: return null;
        }
    }
